 ...
 */
function draw(speedometerValue, iconStates) {
    clearCanvas();
    const mCircle = new MainCircle(options);
    mCircle.draw();
    // Draw speed marks and labels (0, 10, ..., 240)
    const markCount = 25; // 0 to 240 by 10
    const degAngle = 240; // total angle for speedo
    const startAngle = 150; // start at 150deg, end at -90deg
    const marksAngles = mCircle.calcMarksAngles(markCount, degAngle, startAngle);
    mCircle.drawMarks(marksAngles, 7.5, 2, true, 4);
    // Draw speed labels
    ctx.save();
    ctx.translate(mCircle.x, mCircle.y);
    ctx.font = 'bold 16px Arial';
    ctx.fillStyle = '#fff';
    for (let i = 0; i < markCount; i++) {
        const angle = marksAngles[i];
        const radius = mCircle.radius - 32;
        const x = Math.cos(angle) * radius - 10;
        const y = Math.sin(angle) * radius + 6;
        ctx.fillText((i * 10).toString(), x, y);
    }
    ctx.restore();
    ...
    ctx.fillStyle = '#fff';
    const speedValue = Math.round(speedometerValue * 240);
    ctx.fillText(speedValue.toString(), -ctx.measureText(speedValue.toString()).width / 2, 20);
    ctx.font = 'bold 18px Arial';
    ctx.fillText('km/h', -ctx.measureText('km/h').width / 2, 45);
    ctx.restore();
    // Draw icons
    mCircle.drawIcon('engineTemp', iconStates['engineTemp']);
    mCircle.drawIcon('abs', iconStates['abs']);
    mCircle.drawIcon('gas', iconStates['gas']);
    // Draw red pointer
    mCircle.arrowColor = '#ff0000';
    mCircle.drawArrow(speedometerValue, degAngle, 10);
